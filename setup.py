...
try:
    gitversion = subprocess.check_output('git describe --tags --always --first-parent'
                                        .split()).decode().strip().replace('-', '_')
    open("conjure/_version.py", "w").write('__version__ = "{}"\n'.format(gitversion))
    open("httpremoting/_version.py", "w").write('__version__ = "{}"\n'.format(gitversion))
